      ...
      {models.map((model) => (
        <SectionCard key={model.id} className="flex h-full flex-col p-6">
          <div className="flex items-center justify-between gap-4">
            <span className="text-xs font-bold uppercase tracking-wide text-[#18181b]">{model.category || 'Systematic strategy'}</span>
            <span className="rounded-md border border-[#e4e4e7] bg-[#fafafa] px-2 py-1 text-xs font-semibold text-[#18181b]">{model.accessStatus || 'Not reported'}</span>
          </div>
          <h2 className="mt-4 text-2xl font-semibold text-[#09090b]">{model.name}</h2>
          <p className="mt-3 min-h-20 text-sm leading-6 text-[#52525b]">{model.description}</p>
          ...
          <Link href={`/strategies/${model.slug}`} className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[#18181b] hover:underline">
...
function Mini({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="rounded-md border border-[#e4e4e7] bg-[#fafafa] p-3"><span className="block h-4 w-4 text-[#18181b] [&>svg]:h-4 [&>svg]:w-4">{icon}</span><div className="mt-3 text-xs uppercase tracking-wide text-[#71717a]">{label}</div><div className="mt-1 truncate text-sm font-semibold text-[#09090b]">{value}</div></div>;
}
