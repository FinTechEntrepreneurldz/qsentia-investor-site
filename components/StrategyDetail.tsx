...
export default function StrategyDetail({ slug }: { slug: string }) {
  ...
  if (isLoading && !data) return <ApiLoadingPanel title="Loading fund strategy evidence" />;
  ...
  return <div className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
    ...
    <div className="grid content-start gap-5"><SectionCard className="p-6"><div className="text-xs font-bold uppercase tracking-wide text-[#647269]">Reported performance fields</div><div className="mt-5 grid grid-cols-2 gap-3"><Metric icon={<Activity />} label="Annualized return" value={pct(model.performance?.annualizedReturn)} /><Metric icon={<Gauge />} label="Sharpe ratio" value={num(model.performance?.sharpeRatio)} /><Metric icon={<ShieldCheck />} label="Max drawdown" value={pct(model.performance?.maxDrawdown)} /><Metric icon={<Droplets />} label="Hit rate" value={pct(model.performance?.winRate)} /></div></SectionCard><SectionCard className="p-6"><div className="flex items-center gap-2 text-[#3d52da]"><CircleDollarSign className="h-4 w-4"/><span className="text-xs font-bold uppercase tracking-wide">Investor diligence note</span></div><p className="mt-4 text-sm leading-6 text-[#5a685f]">Figures are rendered from current repository-backed telemetry for strategy review. Unreported fund, commercial, and risk fields remain unreported pending manager publication and investor-relations review.</p><div className="mt-4 flex items-center gap-2 text-xs text-[#647269]"><Clock3 className="h-4 w-4"/>Historical results do not guarantee future performance.</div></SectionCard></div>
  </div>;
}
