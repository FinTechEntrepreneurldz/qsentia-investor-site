    ...
    return (
      <Panel title="Loading API commerce" icon={<RefreshCw className="h-4 w-4 animate-spin" />}>
        <div className="grid gap-3 md:grid-cols-3">
          {Array.from({ length: 3 }).map((_, index) => <div key={index} className="h-24 animate-pulse rounded-md bg-[#f4f4f5]" />)}
        </div>
      </Panel>
    );
  }
              ...
              <table className="w-full min-w-[860px] text-left text-sm">
                ...
                <tbody className="divide-y divide-[#e4e4e7]">
                  {data.customers.map((customer) => (
                    <tr key={customer.id}>
                      <td className="py-3"><div className="font-semibold text-[#09090b]">{customer.organization}</div><div className="mt-1 text-xs text-[#71717a]">{customer.primaryContact} · {customer.email}</div></td>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <Panel title="Issue API credential" icon={<KeyRound className="h-4 w-4" />}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2 rounded-md border border-[#e4e4e7] bg-[#fafafa] p-3 text-xs leading-5 text-[#71717a]">Only a SHA-256 hash is retained. The full credential is returned once after issuance.</div>
