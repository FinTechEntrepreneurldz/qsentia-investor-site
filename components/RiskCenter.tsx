...
export default function RiskCenter() {
  ...
  return <div className="grid gap-6"><SectionCard className="overflow-hidden"><div className="grid border-b border-[#e4e4e7] bg-[#fafafa] sm:grid-cols-4"><Summary label="Account status" value={report(d.account_status)}/><Summary label="Current contracts" value={report(d.current_contracts)}/><Summary label="Target contracts" value={report(d.target_contracts)}/><Summary label="Orders submitted" value={report(data.latest?.submittedOrderCount)}/></div><div className="grid gap-px bg-[#e4e4e7] md:grid-cols-2">{controls.map((c)=><div key={c.label} className="bg-white p-5"><div className="flex items-start gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-[#f4f4f5] text-[#18181b] [&>svg]:h-4 [&>svg]:w-4">{c.icon}</span><div><div className="text-sm font-semibold text-[#09090b]">{c.label}</div><div className="mt-1 text-lg font-semibold text-[#18181b]">{c.value}</div><p className="mt-2 text-xs leading-5 text-[#71717a]">{c.evidence}</p></div></div></div>)}</div></SectionCard><div className="grid gap-6 lg:grid-cols-3"><Evidence title="Position reconciliation" value={`${data.positions?.length || 0} position row(s)`} body="Count returned by the active model source."/><Evidence title="Order reconciliation" value={`${data.submittedOrders?.length || 0} submitted row(s)`} body={`${data.plannedOrders?.length || 0} planned order row(s) returned.`}/><Evidence title="Realism monitoring" value={report(data.latest?.realismWarningCount)} body="Warnings from source telemetry; absence is shown as not reported."/></div><p className="text-xs leading-5 text-[#71717a]">This page distinguishes observed states from configured policy limits. A missing limit is not treated as a passing control. Production execution requires documented limits, broker-side controls, reconciliation, and an independently tested kill switch.</p></div>;
}
function Summary({label,value}:{label:string;value:string}){return <div className="border-b border-[#e4e4e7] p-4 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0"><div className="text-xs uppercase tracking-wide text-[#71717a]">{label}</div><div className="mt-2 truncate font-semibold text-[#09090b]">{value}</div></div>}
function Evidence({title,value,body}:{title:string;value:string;body:string}){return <SectionCard className="p-5"><div className="text-xs font-bold uppercase tracking-wide text-[#71717a]">{title}</div><div className="mt-3 text-2xl font-semibold text-[#09090b]">{value}</div><p className="mt-2 text-sm leading-6 text-[#52525b]">{body}</p></SectionCard>}
